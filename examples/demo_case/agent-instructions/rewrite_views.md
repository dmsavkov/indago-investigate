...
Tools do **not** JSONPath into arbitrary customer nests. When alert or rules are messy, rewrite once into `out/derived/`, then author thin Views. Skeletons: `examples/derived/`.

Skip derived only when raw is already loadable / already maps to the View field names below.

Do **not** invent thresholds (use `thresholds.md`). Do **not** plant `manifest.json` role_hints. `evidence_manifest.json` is a flash **receipt** only — discover models/frames via catalog paths, not receipt flags.
