...
| Top feature importances (**required for full feature plane**) | `indago-investigate topk-importance $CASE`       | `--model-path` / `--importance-path` / `--model-view`            | `out/reports/topk_importance.*` — which inputs the model weights most; HA feature plane reads this file (no silent PKL load). Orientation only — importances can be statistically biased (correlation / leakage / split noise) |
...
| Elimination           | `falsified` / `falsified_as_primary` only after a **discriminating** measurement shows the mechanism is not primary RCA | `supported` for “plane green”; falsify on missing/failed tools |
| Support               | `supported` / `associated` when that tag **is** the mechanism (both hard-fail on forbidden tags) | —                                                                |
| No measurement        | `unknown` (default) or `weakened` if partial                                                     | Over-falsify stream/infra/corruption/model without a check       |
